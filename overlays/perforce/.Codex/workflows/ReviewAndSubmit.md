...
1. **Run repo preflight validators first** — run any project preflight checks (e.g. `.Codex/scripts/check-uproject-assoc.sh` when the `ue` overlay is installed) and **abort the submit on any non-zero exit**, reporting the failure so it can be fixed before retrying.
