...
## Mechanism — ensure the schemas are loaded first
The Unreal Engine MCP tools may be **deferred** in the host harness (Claude Code surfaces them by name in the deferred-tool list; schemas are not preloaded). Calling them before loading the schema fails with `InputValidationError`. The Claude-Code equivalent is:
...
If your harness uses a different MCP loading mechanism, apply the equivalent step before calling these tools. Load only the subset you need for the current task.
...
- Don't skip the MCP-load step and call `mcp__unreal-engine__*` directly — the call will fail.
