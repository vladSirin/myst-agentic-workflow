...
|---|---|---|---|---|---|
| `skills/` | 30 | convention (no `skills` key in `.claude-plugin/plugin.json`) | **declared** — `"skills": "./skills/"` | **declared** — `skills.paths` in the user's global `opencode.json` (written by `setup-devkit.ps1`) points at the dedicated clone's `skills/`; scanned `**/SKILL.md`, unknown frontmatter keys ignored, malformed skills log-and-skip | declared (Codex, OpenCode) / convention (Claude). OpenCode `disable-model-invocation` is NOT honoured — restored as a `permission.skill` ask-map the script writes |
...
|---|---|---|
| Reviewer agents not installed (any tool) | the **`review-changes`** skill — runs `review-and-submit`'s two axes (Standards + Spec) inline, sequentially, ending with the per-axis `Verdict:` line the protocol records. Since v4.48.0 it is **purely** the no-subagent path (the fast path and the tiers are gone), but it must NOT be retired: Codex has no Agent tool, so deleting it removes that tool's only review path | `review-and-submit` SKILL.md, and the Codex plugin description |
