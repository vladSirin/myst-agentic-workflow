...
if ($Mode -eq "Write") {
    ...
    # Not piped to Out-Null: the preflight's report-only WARN lines (check 5b) are
    # the only place orphaned scaffold files surface, and swallowing them here hid
    # them in the one context where they matter -- the write gate.
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $preflight -TargetRoot $TargetRoot -PackageRoot $PackageRoot
...
"@
        ...
        if ($UsePerforce -and $targetCL -eq 'new') {
            ...
            $specText = New-P4ChangeSpecText -TagPrefix $clTag -ScriptVersion $ScriptVersion -FileCount $Changes.Count
            $specPath = Join-Path ([System.IO.Path]::GetTempPath()) ("scaffold-cl-" + [System.Guid]::NewGuid().ToString('N') + ".spec")
            try {
                Write-P4SpecFile -Path $specPath -Text $specText
                # `cmd /c` for the redirect: '<' is a reserved, unimplemented operator
                # in PowerShell (5.1 and 7 alike) -- `p4 change -i < $f` is a PARSE
                # error, and piping instead reintroduces the BOM this avoids.
                $created = & cmd /c "p4 change -i < ""$specPath"""
            } finally {
            ...
            # The spec carries no Files: section, so the new CL must be empty. Assert it
            # rather than assume: this is the last line of defence against inheriting the
            # user's default changelist, and it closes the window between the preflight's
            # default-CL check and this moment.
            $prevEA = $ErrorActionPreference; $ErrorActionPreference = 'SilentlyContinue'
            $newClOpened = @(& cmd /c "p4 opened -c $targetCL 2>nul" | Where-Object { $_ -match 'change ' })
            $ErrorActionPreference = $prevEA
            if ($newClOpened.Count -gt 0) {
                Write-Error "CL $targetCL is not empty at creation ($($newClOpened.Count) file(s)); refusing to write into a changelist that already holds files."
                exit 2
            }
            Write-Output "  Created CL $targetCL for write phase (verified empty)."
        }
