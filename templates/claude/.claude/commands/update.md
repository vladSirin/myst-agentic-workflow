...
- **Preflight check 2** (hash mismatch on `opencode.json`) — the documented
  `+w` always-writable pitfall. Suggest skipping that file or running
  `promote.ps1` to push the local edit upstream first.
